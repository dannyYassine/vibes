...
- `agent`: built by `build_agent(api_key, model_name)` — `create_agent` with all 10 tools, `SYSTEM_PROMPT`, and `ConfirmToolMiddleware`
...
7. **Before execution, every tool call hits `ConfirmToolMiddleware.wrap_tool_call`** (unless `CONFIRM_TOOL_CALLS` is `0`/`false`): prints `[tool <name>] <args>` and waits — `y` runs it, `e` replaces args with user JSON, `c`/blank/EOF returns an error `ToolMessage("[cancelled] ...")` with no side effects.
8. `updates` stream: closed `AIMessage` captured as `last_ai`; `ToolMessage` results printed as `[tool <name> result] <text>` (truncated 200 chars).
9. Final `AIMessage` appended to history. On exception: print `[error] <err>`, pop the offending human message, continue loop.
